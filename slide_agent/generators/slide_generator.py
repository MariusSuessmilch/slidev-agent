...
class SlideGenerator:
    ...
    def _clean_code_blocks(content: str) -> str:
        ...
        languages = [
            'javascript', 'typescript', 'python', 'html', 'bash', 'yaml',
            'json', 'java', 'cpp', 'css', 'sql', 'xml', 'c'
        ]
    ...
    def _clean_duplicate_titles(content: str, slide_title: str) -> str:
        ...
        for line in lines:
            ...
            if (line_stripped.lower() == f"title: {slide_title.lower()}" or
                line_stripped.lower() == slide_title.lower() or
                line_stripped.lower() == f"# {slide_title.lower()}" or
                line_stripped == f"# {slide_title}" or
                line_stripped.lower().startswith(f"# {slide_title.lower()}") or
                line_stripped.startswith(f"# {slide_title}")):
                continue
    ...
    def _limit_slide_lines(content: str, max_lines: int = 10) -> str:
        ...
        if any(word in last_line.lower() for word in ['zusammenfassung', 'fazit', 'ausblick', 'zukunft', 'wichtig']):
    ...
    def _prepare_slide_data(self, slide: SlideSpec, is_first: bool = False) -> dict[str, Any]:
    ...
    def _extract_code_data(self, content: str) -> dict[str, Any]:
        ...
        valid_languages = ["python", "javascript", "typescript", "java", "cpp", "c", "sql", "bash", "html", "css", "json", "yaml", "xml"]
    ...
    def _extract_bullets_data(self, content: str) -> dict[str, Any]:
        ...
        for line in lines:
            ...
            if (line.startswith("- ") or line.startswith("* ")) and not line.startswith("  "):
                ...
                if bullet_text and not bullet_text.startswith("-") and not bullet_text.startswith("*"):
                    bullet_points.append(bullet_text)
            elif line and not line.startswith("#") and not line.startswith("  "):  # Skip indented lines
                text_content.append(line)

        return {
            "bullet_points": bullet_points[:5] if bullet_points else None,  # Limit to 5 bullets
    ...
    def _extract_comparison_data(self, content: str) -> dict[str, Any]:
        ...
        parts = re.split(r"\s+(vs\.?|compared to|versus)\s+", content, flags=re.IGNORECASE)
